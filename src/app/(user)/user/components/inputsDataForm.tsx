...
export default function InputsDataForm({ label, configName, id }: { label: any, configName: any, id?: string }) {
    return (
        <>

            <div className="relative mb-8">
                <input
                    className={`w-full px-4 h-12 rounded-lg text-xs placeholder-transparent border-b-2 text-slate-700 border-gray-300 peer focus:outline-none focus:border-purple-600`}
                    placeholder={label}
                    {...configName}
                    id={id}
                    // value={pathname.includes('edit') ? 'true' : ""}
                    onKeyUp={(event) => {
