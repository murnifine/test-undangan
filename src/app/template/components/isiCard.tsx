            ...
            <div className="  sm:grid sm:grid-cols-2 lg:grid-cols-3 flex-col  flex md:flex-row  gap-8 md:gap-10 mt-2  py-5  w-full h-full overflow-y-scroll">
                {dataTemplates.length > 0 ?
                    dataTemplates.slice(0, 3).map((template) => {
                        ...
                        return (
                            <div className="flex flex-col gap-4 justify-center items-center w-full md:w-80 h-[470px] bg-white border px-5 py-4 rounded-lg shadow-lg transition ease-in-out delay-150  hover:-translate-y-1  hover:bg-slate-50 duration-300 text-slate-800 ">
                                <span className="text-lg capitalize font-semibold">{template.nama?.replaceAll('-', ' ')}</span>
                                ...
                                <div className="flex flex-col justify-center items-center gap-2 w-full">
                                    {template.discount &&
                                        <div className="flex gap-4">
                                            <span className="px-4 py-1 rounded-md font-semibold text-white bg-sky-400">Rp {template.harga}</span>
                                            <span className="px-4 py-1 rounded-md font-semibold text-white bg-red-400">{template.discount} % OFF</span>
                                        </div>
                                    }

                                    <span className="text-xl font-bold">
                                        {price === 0 ? 'Gratis' : `Rp ${price ? price.toLocaleString('id-ID') : 0}`}
                                    </span>
                                ...
                                <div className="w-full">
                                    <div className="flex justify-between ">
                                        ...
                                        <Link href={`/user/create?templateId=${template.id}`} className="flex gap-1 py-2 px-2 items-center rounded-md  bg-sky-500 text-white shadow-md hover:bg-red-200 hover:text-sky-600">
